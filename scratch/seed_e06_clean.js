...
async function seedE06Clean() {
  try {
    console.log('=== SEEDING DEPARTMENT E06 & TIMETABLE SLOTS ===\n');

    // 1. Ensure Department E06
    let deptRes = await db.query("SELECT id FROM departments WHERE UPPER(code) = 'E06'");
    let deptId;
    if (deptRes.rows.length === 0) {
      const ins = await db.query("INSERT INTO departments (code, name) VALUES ('E06', 'B.Tech (ECE)') RETURNING id");
      deptId = ins.rows[0].id;
      console.log(`Department E06 created: ${deptId}`);
    } else {
      deptId = deptRes.rows[0].id;
      console.log(`Department E06 exists: ${deptId}`);
    }
    ...
    for (const sub of subjectsData) {
      ...
      if (subCheck.rows.length === 0) {
        ...
        console.log(`  + Subject inserted: ${sub.code} (${insSub.rows[0].id})`);
      } else {
        subjectIdMap[sub.code] = subCheck.rows[0].id;
        console.log(`  = Subject exists: ${sub.code} (${subCheck.rows[0].id})`);
      }
    }
    ...
    console.log('\n=== E06 SEEDING SUCCESSFUL: 15 SUBJECTS & 42 TIMETABLE SLOTS INSTALLED ===');
