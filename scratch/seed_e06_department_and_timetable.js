...
async function seedE06() {
  const client = await db.pool.connect();
  try {
    console.log('================================================================');
    console.log('=== SEEDING DEPARTMENT E06 (B.Tech ECE) & TIMETABLE (SEM 5) ===');
    console.log('================================================================\n');

    await client.query('BEGIN');

    // 1. Ensure Department E06 exists
    let deptRes = await client.query("SELECT id FROM departments WHERE UPPER(code) = 'E06'");
    let deptId;
    if (deptRes.rows.length === 0) {
      const insDept = await client.query(
        "INSERT INTO departments (code, name) VALUES ('E06', 'B.Tech (ECE)') RETURNING id"
      );
      deptId = insDept.rows[0].id;
      console.log(`[DEPARTMENT CREATED]: E06 - B.Tech (ECE) (ID: ${deptId})`);
    } else {
      deptId = deptRes.rows[0].id;
      console.log(`[DEPARTMENT EXISTS]: E06 - B.Tech (ECE) (ID: ${deptId})`);
    }

    // 2. Define Master Subjects for E06 Semester 5
    const subjectsData = [
    ...
    for (const sub of subjectsData) {
      // Check if master subject exists for E06 sem 5
    ...
    // 3. Define Timetable Grid (6 Days x 7 Periods)
    const timeSlots = {
    ...
    // Delete existing E06 timetable slots to allow clean seed
    await client.query(
    ...
    for (const slot of timetableGrid) {
      ...
      if (!subId) {
        throw new Error(`Subject ID not found for code: ${slot.code}`);
      }
    ...
    console.log('[CHECKPOINT 1 PASSED]: 42 Timetable slots inserted successfully.');
