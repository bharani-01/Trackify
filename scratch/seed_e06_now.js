...
async function seedE06Now() {
  try {
    ...
    // 2. Master Subjects Data for E06 Semester 5
    const subjectsData = [
    ...
    for (const sub of subjectsData) {
      ...
      if (subCheck.rows.length === 0) {
        ...
        console.log(`  + Master Subject Inserted: ${sub.code} - ${sub.name}`);
      } else {
        subjectIdMap[sub.code] = subCheck.rows[0].id;
        console.log(`  = Master Subject Found: ${sub.code}`);
      }
    }
    ...
    const delRes = await db.query(`DELETE FROM timetable WHERE department_id = $1 OR UPPER(department) = 'E06'`, [deptId]);
    console.log(`\n[PURGED PREVIOUS E06 SLOTS]: ${delRes.rowCount} deleted.`);

    console.log('[SEEDING TIMETABLE SLOTS]: Inserting 42 slots...');
    ...
    console.log('\n================================================================');
    console.log('=== E06 SEEDING COMPLETED WITH 100% SUCCESS ===');
    console.log('================================================================');
